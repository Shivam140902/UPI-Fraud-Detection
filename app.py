# app.py - SecurePay
# Single-file Flask app with SQLite and optional ML (auto-disabled if files missing)
# Ready to paste and deploy. Keep your templates/ and static/ folders in place.
...
# ---------------- ML loading ----------------
def load_ml_assets():
    """Load scaler (fit from CSV) and Keras model if available. If missing, ML is disabled but app still runs."""
    global model, scaler

    # Load scaler (fit on dataset if available)
    try:
        if os.path.exists(DATASET_CSV):
            ...
            if dataset.shape[1] >= 10:
                ...
                scaler = None
                print(f"Dataset found but columns < 10. Scaler not fitted.")
        else:
            scaler = None
        ...
        try:
            if form_type == 'user':
                ...
                if not username or not password:
                    ...
                    cur.close()
                    return render_template('login.html')
                # simple admin check (change in production)
...
# ===== PART 2 =====
# Registration and user routes are below (kept concise)
...
def user_profile_page():
    ...
    try:
        ...
        flash(f'Database error: {e}', 'danger')
        print("user_profile_page error:", e)
        traceback.print_exc()
    finally:
        try:
            cur.close()
...
def user_transactions_page():
    ...
    try:
        ...
        flash(f'Database error: {e}', 'danger')
        print("user_transactions_page error:", e)
        traceback.print_exc()
    finally:
        try:
            cur.close()
...
def user_pay():
    ...
    try:
        ...
        trans_year = now.year
        category = int(merchant_details[0])

        dob_str = user_details[0]
        try:
        ...
        state = int(user_details[1]) if user_details[1] is not None else 0
        zip_code = int(user_details[2]) if user_details[2] is not None else 0
...
def admin_users_page():
    ...
    try:
        ...
        flash(f'Database error: {e}', 'danger')
        print("admin_users_page error:", e)
        traceback.print_exc()
    finally:
        try:
            cur.close()
...
def admin_merchants_page():
    ...
    try:
        ...
        flash(f'Database error: {e}', 'danger')
        print("admin_merchants_page error:", e)
        traceback.print_exc()
    finally:
        try:
            cur.close()
...
def admin_transactions_page():
    ...
    try:
        ...
        flash(f'Database error: {e}', 'danger')
        print("admin_transactions_page error:", e)
        traceback.print_exc()
    finally:
        try:
            cur.close()
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if "UNIQUE constraint failed" in str(e):
                ...
                flash(f'Database error: {e}', 'danger')
            print("admin_create_account IntegrityError:", e)
        except Exception as e:
            flash(f'An unexpected error occurred: {e}', 'danger')
            print("admin_create_account error:", e)
            traceback.print_exc()
        finally:
            try:
                cur.close()
...
def merchant_dashboard():
    ...
    try:
        ...
        flash(f'Database error: {e}', 'danger')
        print("merchant_dashboard error:", e)
        traceback.print_exc()
    finally:
        try:
            cur.close()
...
def merchant_setup():
    ...
    try:
        ...
        flash(f'Database error: {e}', 'danger')
        print("merchant_setup select error:", e)
        try:
            cur.close()
        except Exception:
            pass
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if "UNIQUE constraint failed" in str(e):
                ...
                flash(f'Database error: {e}', 'danger')
            print("merchant_setup IntegrityError:", e)
        except Exception as e:
            flash(f'Unexpected error: {e}', 'danger')
            print("merchant_setup error:", e)
            traceback.print_exc()
        finally:
            try:
                cur.close()
...
if __name__ == '__main__':
    try:
        ...
        print("🚀 SecurePay app started successfully with SQLite DB:", SQLITE_DB_PATH)
